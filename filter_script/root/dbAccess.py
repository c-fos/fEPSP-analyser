...
class Mysql_writer:
    ...
    def dbWriteResponse(self,tmpObject):
        ...
        cursor.execute("INSERT INTO responses(number,numberofspikes,length,record_idrecord,vpsp)\
                             VALUES(%s,%s,%s,%s,%s)", (str(rNumber),str(nOfSpikes),str(rLength),str(idRecord),str(vpsp)))
        self.conn.commit()
        cursor.close()
